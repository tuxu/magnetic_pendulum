...
void rk45(const float4 *y, const float t, const float dt, float4 *yout,
          ...
          __local float alphas[ALPHAS_LEN],
...
__kernel void map_magnets(__global float *coords,
                          ...
                          const float min_kin,
                          const unsigned int max_iterations,
                          __local float l_alphas[ALPHAS_LEN],
                          __local float l_rns[RNS_LEN]) {
    ...
    if (i < count) {
        ...
        // Fill local cache of parameters.
        size_t j = get_local_id(0);
        if (j < n_magnets) {
            l_alphas[j] = g_alphas[j];
            l_rns[3 * j + 0] = g_rns[3 * j + 0];
            l_rns[3 * j + 1] = g_rns[3 * j + 1];
            l_rns[3 * j + 2] = g_rns[3 * j + 2];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
        
        int magnet = find_magnet(phi, theta, friction, exponent, n_magnets,
                                 l_alphas, l_rns, time_step, min_kin,
                                 max_iterations);
        magnets[ind] = magnet;
    }
}
